...
| `/app/policy/[id]` | Policy detail. Has the real **"File a claim"** panel (`ClaimPanel.tsx`) — paste a transaction hash, watch the adjuster's checklist and reasoning come back live, reusing `TxChecklist` from the bind flow. |
| `/demo/scam-airdrop` | **The centerpiece of the hackathon video.** A staged, clearly-labeled "phishing site" that reproduces a real wallet-drainer attack against `contracts/src/mocks/MockDrainer.sol` — a real approval you sign, a real drain, then a real claim submitted to the adjuster and (if approved) paid. See root `README.md`'s demo shot list. |
