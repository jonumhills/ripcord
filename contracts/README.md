...
`src/mocks/MockDrainer.sol` reproduces the actual mechanics of a wallet-drainer phishing site — approve a spending cap, get pulled via `transferFrom` — rather than faking the drain some other way. Deployed on Arc testnet: **`0x63c680e5dD8691A851E3b6946F28804682B77301`**. `drain(token, victim)` is deliberately permissionless (anyone can call it once a victim has approved it) — a real drainer bot does the same thing, it doesn't wait for permission either. See `web/app/demo/scam-airdrop/page.tsx` for the page that drives this, and root `README.md` for the full demo shot list.
...
**Drained funds forward straight into `PolicyVault`, not this contract (2026-09-13).** Originally `drain()` just pulled funds to itself, with no withdraw function — every demo run permanently stranded real testnet USDC there, which is exactly how the vault ended up underfunded for a payout earlier. Now the constructor takes an immutable `vault` address (set to `PolicyVault` at deploy time), and `drain()` does two real transfers in one call: `victim -> MockDrainer` (unchanged — this is still the leg `claimsAdjuster.ts` matches against `DEMO_FLAGGED_ADDRESSES`, since it only inspects the *first* Transfer whose `from` is a covered address), then `MockDrainer -> vault`. Verified live: approved + drained 0.05 USDC from the deployer wallet, confirmed on Arcscan as two Transfer events plus the contract's own `Drained` event, `PolicyVault`'s balance went up by exactly 0.05 USDC and `MockDrainer`'s stayed at zero, and a claim filed against that same tx hash still correctly matched the destination as flagged (the forwarding leg doesn't interfere with the check). Redeploying MockDrainer for any reason means a new address — update `NEXT_PUBLIC_MOCK_DRAINER_ADDRESS` (web) and `DEMO_FLAGGED_ADDRESSES` (backend) together, or the two will disagree about what counts as the flagged destination.
